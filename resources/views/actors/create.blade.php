...
<form action="{{ route('actors.store') }}" method="POST">
    ...
    <div class="input-group input-group-sm my-5">
        ...
        <input type="text" name="surname" class="form-control @error('surname') is-invalid @enderror" value="{{old('surname')}}" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
        @error('surname')
        <div class="invalid-feedback">
          {{ $message }}
        </div>
